...
def banner():
	...
	sol().print(cik2)
	ban='''______   __ _    _____ ___ ___    __  ______
/ ___\ \ / // \  |  ___|_ _|_ _|   \ \/ /  _ \
\___ \\ V // _ \ | |_   | | | |_____\  /| | | |
 ___) || |/ ___ \|  _|  | | | |_____/  \| |_| |
...
def dump_massal():
	...
	for userr in uid:
		try:
			col = ses.get('https://graph.facebook.com/v2.0/'+userr+'?fields=friends.limit(5000)&access_token='+tokenku[0],cookies={'cookie': kukis}).json()
...
def dump_pengikut():
	...
	pil = input(x+'['+p+'f'+x+'] INPUT TARGET ID : ')
	try:
		koh2 = requests.get('https://graph.facebook.com/'+pil+'?fields=subscribers.limit(99999)&access_token='+tokenku[0],cookies={'cookie': kukis}).json()
...
def dump_likes():
	...
	try:
		koh2 = requests.get('https://graph.facebook.com/v1.0/'+pil+'?fields=likes.limit(10000)&access_token='+tokenku[0],cookies={'cookie': kukis}).json()
...
def pulkanid(linkmem):	
	member=ses.get(linkmem,cookies={'cookie': kukis},headers={'user_agent': ''}).text
	...
	if "Lihat Selengkapnya" in member:
		...
		def geh(gey):
			try:
				a=requests.get(gey,cookies={'cookie': kukis}).text
